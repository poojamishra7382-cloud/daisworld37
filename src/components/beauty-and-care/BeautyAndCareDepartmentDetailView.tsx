...
import { BeautyAndCareDepartment, beautyAndCareDepartments } from '@/data/beautyAndCareDepartments';

interface Props {
  department: BeautyAndCareDepartment;
}

export default function BeautyAndCareDepartmentDetailView({ department }: Props) {
  const [selectedRoleIndex, setSelectedRoleIndex] = useState(0);

  const activeRole = department.roles[selectedRoleIndex] || department.roles[0];
  const otherDepartments = beautyAndCareDepartments.filter((d) => d.id !== department.id);
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs font-semibold uppercase tracking-wider mb-4">
            ...
            Beauty & Care Aesthetics Department
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-14 items-center">
            ...
            <div className="lg:col-span-7">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-50 border border-pink-100 text-pink-700 text-xs font-bold uppercase tracking-wider mb-4">
                <Briefcase className="w-3.5 h-3.5" />
                Department Overview
              </div>

              <h2 className="text-2xl sm:text-4xl font-black text-slate-900 leading-tight mb-5">
                European & Global Luxury Standards in{' '}
              ...
              {/* Perks Highlight Box */}
              <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-pink-50/80 to-rose-50/40 border border-pink-200/80">
                <div className="flex items-start gap-3.5">
                  <div className="w-10 h-10 rounded-xl bg-pink-600 text-white flex items-center justify-center flex-shrink-0 shadow-md">
                    <ShieldCheck className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 mb-1">
                      Salon & Spa Relocation Package
                    </h3>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-slate-50 border border-slate-200/80 rounded-3xl p-6 shadow-sm">
                <h3 className="text-lg font-black text-slate-900 mb-4 flex items-center gap-2">
                  ...
                  Why Beauty Talents Choose Dais World Endeavor
                </h3>

                <ul className="space-y-3">
                  {[
                    'Direct contracts with premier European luxury salons & 5-star hotel spas',
                    'High base salary + 100% direct tips & retail sales commission shares',
                    'Accreditation support for CIDESCO, CIBTAC, and ITEC European recognition',
                    'Flight tickets, visa sponsorship, and furnished accommodation assistance',
                    'Accelerated European career growth and international celebrity exposure',
                  ].map((item) => (
                ...
                >
                  Apply for Beauty Talent Evaluation
                </button>
              </div>
            </div>
          </div>
      ...
      <section id="job-roles" className="py-16 sm:py-24 bg-gradient-to-b from-slate-50 via-white to-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
            <p className="text-pink-600 font-bold text-xs sm:text-sm uppercase tracking-widest mb-2.5">
              Available Positions
            </p>
            ...
            <p className="text-slate-500 text-sm sm:text-base leading-relaxed">
              Explore open luxury salon, spa, and beauty studio positions with verified employer sponsorships.
            </p>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xl overflow-hidden">
            ...
            <div className="bg-slate-900 p-6 sm:p-8 text-white">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-pink-300 border border-white/20 text-xs font-semibold uppercase tracking-wider mb-2">
                ...
                <ul className="space-y-3">
                  {activeRole.responsibilities.map((resp, i) => (
                    <li key={i} className="flex items-start gap-3 text-xs sm:text-sm text-slate-600">
                      <span className="w-5 h-5 rounded-full bg-pink-50 text-pink-700 font-bold text-[11px] flex items-center justify-center flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                      <span>{resp}</span>
                    </li>
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-base font-bold text-slate-900 mb-3 flex items-center gap-2">
                    ...
                    Required Qualifications & Diplomas
                  </h4>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeRole.skills.map((skill) => (
                      <span
                        key={skill}
                        className="px-3 py-1 rounded-lg bg-pink-50 text-pink-800 border border-pink-200 text-xs font-medium"
                      >
                        {skill}
                      </span>
                    ))}
      ...
      <section className="py-16 sm:py-20 bg-white border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-10 lg:gap-14">
            {/* Responsibilities */}
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-50 text-pink-700 text-xs font-bold uppercase tracking-wider mb-4">
                <FileCheck className="w-3.5 h-3.5" />
                Department Core Scope
              </div>
              <h3 className="text-2xl sm:text-3xl font-black text-slate-900 mb-6">
                Client Care & Aesthetic Duties
              </h3>
              ...
              <div className="space-y-4">
                {department.keyResponsibilities.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-pink-50/50 hover:border-pink-100 transition-colors"
                  >
                    <div className="w-8 h-8 rounded-xl bg-pink-600 text-white font-bold text-xs flex items-center justify-center flex-shrink-0 shadow-sm">
                      {idx + 1}
                    </div>
            ...
            <div>
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3">
                  Artistic & Technical Competencies
                </h4>
                <div className="flex flex-wrap gap-2">
                  {department.requiredSkills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3.5 py-1.5 rounded-xl bg-pink-50 text-pink-800 border border-pink-200 text-xs font-semibold"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-4">
              Career Growth & International Salon Perks
            </h2>
            <p className="text-slate-500 text-sm sm:text-base">
              Working with top European luxury salons and 5-star resort spas provides world-class portfolios, high tip earnings, and global prestige.
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {department.careerOpportunities.map((opp, idx) => (
              ...
              >
                <div>
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-pink-600 to-rose-500 text-white flex items-center justify-center mb-5 shadow-md">
            ...
            {otherDepartments.map((d) => {
              ...
              return (
                <Link
                  key={d.id}
                  to={d.route}
                  className="group bg-slate-50 hover:bg-white rounded-3xl p-6 border border-slate-200 hover:border-pink-300 hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5 flex flex-col justify-between"
                >
                  <div>
                    <div
                    ...
                    <h4 className="font-black text-lg text-slate-900 mb-1 group-hover:text-pink-600 transition-colors">
                      {d.title}
                    </h4>
                  ...
                  <div className="flex items-center gap-2 text-pink-600 font-bold text-xs group-hover:gap-3 transition-all pt-2 border-t border-slate-100">
                    Explore Department
