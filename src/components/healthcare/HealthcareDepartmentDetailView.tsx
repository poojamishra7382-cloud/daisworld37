...
import { HealthcareDepartment, healthcareDepartments } from '@/data/healthcareDepartments';

interface Props {
  department: HealthcareDepartment;
}

export default function HealthcareDepartmentDetailView({ department }: Props) {
  const [selectedRoleIndex, setSelectedRoleIndex] = useState(0);

  const activeRole = department.roles[selectedRoleIndex] || department.roles[0];
  const otherDepartments = healthcareDepartments.filter((d) => d.id !== department.id);
  ...
  return (
    <div className="w-full bg-slate-50 min-h-screen">
      ...
      >
        {/* Simple Clean Dark Overlay - No Color Shades */}
      ...
      {/* =========================================================
          3. DEPARTMENT OVERVIEW & LICENSING
      ========================================================= */}
          ...
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-14 items-center">
            ...
            <div className="lg:col-span-7">
              ...
              <h2 className="text-2xl sm:text-4xl font-black text-slate-900 leading-tight mb-5">
                International Career Standards in{' '}
              ...
              {/* Licensing Highlight Box */}
              <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-blue-50/80 to-cyan-50/40 border border-blue-200/80">
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-blue-950 mb-1">
                      International Licensing & Qualification Recognition Support
                    </h3>
                    <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                      {department.licensingHighlights}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-slate-50 border border-slate-200/80 rounded-3xl p-6 shadow-sm">
                ...
                <ul className="space-y-3">
                  {[
                    'Direct employment contracts with accredited hospital groups',
                    'Zero hidden processing charges & transparent recruitment',
                    'Comprehensive language coaching & licensing dossier support',
                    'Full family visa, flight booking & initial accommodation',
                    'Permanent residency (PR) pathway in Europe & the UK',
                  ].map((item) => (
      ...
      {/* =========================================================
          4. AVAILABLE JOB ROLES (INTERACTIVE CARDS)
      ========================================================= */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
            ...
            <p className="text-slate-500 text-sm sm:text-base leading-relaxed">
              Explore currently open clinical designations with verified international hospital sponsorships.
            </p>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xl overflow-hidden">
            ...
            <div className="bg-slate-900 p-6 sm:p-8 text-white">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-cyan-300 border border-white/20 text-xs font-semibold uppercase tracking-wider mb-2">
                ...
                <div className="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl px-5 py-3 text-right">
                  <p className="text-xs text-white/70 font-medium">Estimated Monthly Salary</p>
                ...
                <ul className="space-y-3">
                  {activeRole.responsibilities.map((resp, i) => (
                    <li key={i} className="flex items-start gap-3 text-xs sm:text-sm text-slate-600">
                      <span className="w-5 h-5 rounded-full bg-blue-50 text-blue-600 font-bold text-[11px] flex items-center justify-center flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                      <span>{resp}</span>
                    </li>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeRole.skills.map((skill) => (
                      <span
                        key={skill}
                        className="px-3 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 text-xs font-medium"
                      >
                        {skill}
                      </span>
                    ))}
      ...
      {/* =========================================================
          5. RESPONSIBILITIES & QUALIFICATIONS MATRIX
      ========================================================= */}
            ...
            <div>
              ...
              <div className="space-y-4">
                {department.keyResponsibilities.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-blue-50/50 hover:border-blue-100 transition-colors"
                  >
                    <div className="w-8 h-8 rounded-xl bg-blue-600 text-white font-bold text-xs flex items-center justify-center flex-shrink-0 shadow-sm">
                      {idx + 1}
                    </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {department.requiredSkills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3.5 py-1.5 rounded-xl bg-cyan-50 text-cyan-800 border border-cyan-200 text-xs font-semibold"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {department.careerOpportunities.map((opp, idx) => (
              ...
              >
                <div>
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-600 to-cyan-500 text-white flex items-center justify-center mb-5 shadow-md">
                    <TrendingUp className="w-6 h-6" />
                  </div>
                  <h4 className="font-bold text-slate-900 text-base mb-2">
                    Benefit Advantage {String(idx + 1).padStart(2, '0')}
                  </h4>
                  <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                    {opp}
                  </p>
      ...
      {/* =========================================================
          7. OTHER HEALTHCARE DEPARTMENTS (NAVIGATOR)
      ========================================================= */}
            ...
            {otherDepartments.map((d) => {
              ...
              return (
                <Link
                  key={d.id}
                  to={d.route}
                  className="group bg-slate-50 hover:bg-white rounded-3xl p-6 border border-slate-200 hover:border-blue-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5 flex flex-col justify-between"
                >
                  <div>
                    <div
